...
Each chart's `.schema.yaml` holds the input, draft, indent and output, so the
command takes no flags.

Install the plugin pinned to a released tag. Without `--version` the installer
clones the default branch, which reports the last tag but is whatever was on
...
```bash
cd charts/document-engine && helm schema
cd charts/ai-assistant && helm schema
